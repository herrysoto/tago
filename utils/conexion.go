...
func InitDbOracle() *gorp.DbMap {
	viper.SetConfigName("flatrate") // name of config file (without extension) envoca un archivo json y actualiza la informacion
	viper.AddConfigPath(".")         // path to look for the config file in

	err := viper.ReadInConfig()
	if err != nil {
		fmt.Println("Config not found...flatrate.json")
		fmt.Println(err)
		return nil
	}
