...
import (
	bean "flatrate/entidad"
	utils "flatrate/utils"
	"time"
	//"fmt"
	"log"
...
func Getgrid(c *gin.Context) ([]bean.OperacionServicio,error){
		...
		_,err := utils.DbmapOracle.Select(&opser,`SELECT nvl(NUMNROPIEZA,0) NUMNROPIEZA,VCHNROTRABAJO,VCHCODIGOOPERACION || '-' || CHRCODIGOOPERACIONSERVICIO  CODIGO,VCHCODIGOOPERACION,CHRCODIGOOPERACIONSERVICIO, VCHDESCRIPCION, ROUND(NUMHORASHOMBRE,2) NUMHORASHOMBRE,
                   ...
                   nvl(NUMMANOOBRA,0) NUMMANOOBRA,nvl(NUMREPUESTO,0) NUMREPUESTO,nvl(NUMTOTAL,0) NUMTOTAL,NUMCODIGO
...
//  		var desc bean.OperacionServicio 
...
//          FROM OPERACIONSERVICIOS
//          WHERE TRIM(UPPER(VCHDESCRIPCION)) LIKE '%'|| TRIM(UPPER(:VCHDESCRIPCION)) ||'%'`,desc.VCHDESCRIPCION)
// 		 log.Println(desc)
//  		if err !=nil{
//  			log.Println("error")
//  			log.Println(err)
...
func LISTAR_OPERSERVCONTENIDOS(c *gin.Context)([]bean.OperacionServiciosContenidos,[]bean.OperacionServicio){
	...
	param1 := c.Params.ByName("param1")
		...
		_,err := utils.DbmapOracle.Select(&opserparam1,`SELECT nvl(OPCS.NUMNROPIEZA,0) NUMNROPIEZA,OPC.NUMCODIGOOPERACIONMAESTRA, OPC.VCHCODIGOOPERACION || '-' || OPC.CHRCODIGOOPERACIONSERVICIO  CODIGO,OPCS.VCHNROTRABAJO,TRIM(OPC.VCHCODIGOOPERACION) VCHCODIGOOPERACION,TRIM(OPC.CHRCODIGOOPERACIONSERVICIO) CHRCODIGOOPERACIONSERVICIO,OPCS.VCHDESCRIPCION,
...
func PutOperaServi(c *gin.Context) (bean.UpdateOS, error) {
	...
	_, err := utils.DbmapOracle.Exec(`UPDATE OPERACIONSERVICIOS SET VCHNROTRABAJO=:VCHNROTRABAJO,VCHDESCRIPCION=:VCHDESCRIPCION, NUMPRECIOSUGERIDO=:NUMPRECIOSUGERIDO,
        							...
        							WHERE UPPER(TRIM(CHRCODIGOOPERACIONSERVICIO)) = UPPER(TRIM(:putOpeSer)) AND  UPPER(TRIM(VCHCODIGOOPERACION)) = UPPER(TRIM(:putOpeSer1))`,0/*VCHNROTRABAJO*/,
...
func UpdateGrilla(c *gin.Context)(bean.UpdateGrilla,error){
	var upgrill bean.UpdateGrilla
	c.Bind(&upgrill)
	_, err := utils.DbmapOracle.Exec(`update operacionservicios set NUMPRECIOSUGERIDO=:NUMPRECIOSUGERIDO , NUMTOTAL =:NUMTOTAL , NUMDESCUENTO=:NUMDESCUENTO,NUMHORASHOMBRE=:NUMHORASHOMBRE 
										where UPPER(TRIM(VCHCODIGOOPERACION)) = UPPER(TRIM(:putOpeSer1)) 
										and UPPER(TRIM(CHRCODIGOOPERACIONSERVICIO)) = UPPER(TRIM(:putOpeSer))`,

		upgrill.NUMPRECIOSUGERIDO, upgrill.NUMTOTAL, upgrill.NUMDESCUENTO, upgrill.NUMHORASHOMBRE, upgrill.VCHCODIGOOPERACION, upgrill.CHRCODIGOOPERACIONSERVICIO)
	if err != nil {
		log.Println("error")
		log.Println(err)
	}
	...
	_, err := utils.DbmapOracle.Exec(`INSERT INTO OPERACIONSERVICIOS (CHRCODIGOOPERACIONSERVICIO,VCHCODIGOOPERACION,VCHDESCRIPCION,
        ...
        values (:CHRCODIGOOPERACIONSERVICIO,:VCHCODIGOOPERACION,:VCHDESCRIPCION,:NUMPRECIOSUGERIDO,:NUMPRECIOOFICIAL,:NUMHORASHOMBRE,:NUMDESCUENTO,:CHRESTADO,
			   ...
			   insertOS.CHRCODIGOOPERACIONSERVICIO, insertOS.VCHCODIGOOPERACION, insertOS.VCHDESCRIPCION,
				...
				4/*:NUMPRECIOESTIMADO*/,0/*:NUMNROPIEZA*/,0/*:VCHNROTRABAJO*/,insertOS.NUMCODIGO/*:NUMCODIGO*/,0/*:NUMMANOOBRA*/,0/*:NUMREPUESTO*/,insertOS.NUMTOTAL/*:NUMTOTAL*/)
	...
	_, err := utils.DbmapOracle.Exec(`INSERT INTO OPERACIONSERVICIOS (CHRCODIGOOPERACIONSERVICIO,VCHCODIGOOPERACION,VCHDESCRIPCION,
        ...
        values (:CHRCODIGOOPERACIONSERVICIO,:VCHCODIGOOPERACION,:VCHDESCRIPCION,:NUMPRECIOSUGERIDO,:NUMPRECIOOFICIAL,:NUMHORASHOMBRE,:NUMDESCUENTO,:CHRESTADO,
               :DTECREACION,:NUMTIPOSERVICIO,:DTEMODIFICACION,:NUMPRECIOESTIMADO,:NUMNROPIEZA,:VCHNROTRABAJO,:NUMCODIGO,:NUMMANOOBRA,:NUMREPUESTO,:NUMTOTAL)`, insertOS.CHRCODIGOOPERACIONSERVICIO, insertOS.VCHCODIGOOPERACION, insertOS.VCHDESCRIPCION,
				...
				time.Now()/*:DTEMODIFICACION*/,4/*:NUMPRECIOESTIMADO*/,0/*:NUMNROPIEZA*/,0/*:VCHNROTRABAJO*/,insertOS.NUMCODIGO/*:NUMCODIGO*/,0/*:NUMMANOOBRA*/,0/*:NUMREPUESTO*/,insertOS.NUMTOTAL/*:NUMTOTAL*/)
...
//comienzan las funciones para eliminar un servicio contenido,  se usaran dos métodos, primero un select y luego el delete

// func GetNumMaestra(c *gin.Context)([]bean.NumMaestra,error){
// 	operacionmaestra:= c.Params.ByName("operacionmaestra")
// 	codigooperacion:= c.Params.ByName("codigooperacion")
// 	codoperserv:= c.Params.ByName("codoperserv")
// 	var nummaestra []bean.NumMaestra
// 	_,err := utils.DbmapOracle.Select(&nummaestra, `SELECT X.NUMCODIGOOPERACIONMAESTRA 
// 										FROM OPERACIONSERVICIOSCONTENIDOS X 
// 										WHERE TRIM(X.VCHOPERACIONMAESTRA) = TRIM(:operacionmaestra) 
// 										AND TRIM(X.VCHCODIGOOPERACION) = TRIM(:codigooperacion)
// 										AND TRIM(X.CHRCODIGOOPERACIONSERVICIO) = TRIM(:codoperserv)`,operacionmaestra,codigooperacion,codoperserv)
// 	if err != nil {
// 		log.Println("error")
// 		log.Println(err)
// 		return nil,err
// 	}
// 	return nummaestra,nil
// }

func DeleteContenido(c *gin.Context) {
	nummaestra := c.Params.ByName("nummaestra")
	_, err := utils.DbmapOracle.Exec(`Delete from operacionservicioscontenidos where numcodigooperacionmaestra =:nummaestra`, nummaestra)
	if err != nil {
		log.Println(err)
...
// func DeleteContenido(c *gin.Context)(bean.NumMaestra,error) {
// 	var nummaestra bean.NumMaestra
// 	c.Bind(&nummaestra)
// 	// nummaestra := c.Params.ByName("nummaestra")
// 	_, err := utils.DbmapOracle.Exec(`Delete from operacionservicioscontenidos where numcodigooperacionmaestra =:NUMCODIGOOPERACIONMAESTRA`, nummaestra.NUMCODIGOOPERACIONMAESTRA)
// 	if err != nil {
// 		log.Println("error")
// 		log.Println(err)
// 	}
